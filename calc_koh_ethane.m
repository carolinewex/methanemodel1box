function [box_kOH] = calc_koh_ethane(lat_top, lat_bot)
%************************************************************
%calculates k_OH for ethane
%inputs: 
...
%*************************************************************
   ...
   %global_k_OH_ethane = globalOH.*1e5.*7.66e-12.*exp(-1020./globalT);
   %%from Sander et al. 2015, evaluation #15
    
   
    %global_k_OH_ethane = globalOH.*1e5.*1.64e-12.*exp(-1520./globalT);
    %%from Sander et al., JPL publication pg. 1-11 methyl chloroform 
        %global_k_OH_ethane = globalOH.*1e5.*2.45e-12.*exp(-1775./globalT); %methane
    ...
    k_zonal = nansum(global_k_OH_ethane.*p_frac2, 1)'; %each vertical layer weighted and summed to get 24 zonal k_OH values 
    SA = 4*pi*(6378100^2);
    lat(1) = -90;
    lat(end)= 90;
    lat_i = (-90:1:90)';
